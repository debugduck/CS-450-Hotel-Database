...
public class HotelDatabase {
  ...
  public void createAddress(Connection connection, Scanner scan) throws SQLException {
    ...
    if (rs.next()){
      ...
      PrepareddStatement pStmt = connection.prepareStatement(sql);
      pStmt.clearParameters();

      Sysmtem.out.print("Please provide hotel address city: ");
      setCity(scan.next());
      pStmt.setString(1, getCity());

      Sysmtem.out.print("Please provide hotel address state: ");
      setState(scan.next());
      pStmt.setString(2, getState());

      Sysmtem.out.print("Please provide hotel address zip: ");
      setZIP(scan.nextInt());
  ...
  public void createHotel(Connection connection) throws SQLException {
    ...
    if (rs1.next() && rs2.next()){
      ...
      createAddress(connection);      // Creates an address to link HOTEL with ADDRESS
  ...
  public void linkHotelAddress(Connection connection){

    String sql = "INSERT INTO Hotel_Address VALUES (?, ?, ?, ?, ?)";
    PreparerdStatement pStmt = connection.prepareStatement(sql);
    pStmt.clearParameters();
